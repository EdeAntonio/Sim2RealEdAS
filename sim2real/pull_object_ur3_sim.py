...
class POTUR3Sim (EnviromentAdapter):
    ...
    def __init__(self):
        super().__init__(action_scale = 0.5, action_size = 6, model_path=args.policy_path, robot = UR5Sim("192.168.1.140", 30004, pos_init=[-3.86, 0.3, -2.24, 0.91, 2.22, -1.75]))
        ...
        if len(args.target) == 7:
            ...
            print("Dimensión incorrecta. Debe darse como posicion (3) + cuaternio (4)")
    ...
    def _compute_observation(self) -> np.ndarray:
        ...
        obs[16:22] = self.state.robot.joint_velocities
        obs[32:35] = self.state.tool_pos
        obs[35:38] = self.state.object_pos
        obs[38:44] = self.target

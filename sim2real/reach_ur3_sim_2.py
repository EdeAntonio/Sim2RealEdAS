...
class ReachUR3Sim (EnviromentAdapter):
    ...
    def __init__(self):
        super().__init__(action_scale = 0.5, action_size = 8, model_path=args.policy_path, robot = UR5Sim("192.168.1.140", 30004, pos_init=[-1.57, -1.744, 157, -1.573, -1.573, -1.23]))
    ...
    def _compute_observation(self) -> np.ndarray:
        ...
        obs[6:8] = [0.0, 0.0]
        obs[8:14] = self.state.robot.joint_velocities
        obs[14:16] = [0.0, 0.0]
        obs[16:23] = self.target
        obs[23:29] = self._previous_action
        return obs
